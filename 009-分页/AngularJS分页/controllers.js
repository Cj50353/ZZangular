...
tableApp.controller('TableController', function ($scope, $routeParams, userService) {
    ...
    $scope.setPageNumber = function (pn) {
        if (pn <= 1) {
            $scope.pageNumber = 1;
            location.hash='#/table/1';
        } else if (pn >= $scope.numberOfPages) {
            $scope.pageNumber = $scope.numberOfPages;
            location.hash='#/table/'+$scope.numberOfPages
        } else {
            $scope.pageNumber = pn;
            location.hash='#/table/'+pn;
        }
    };
    $scope.subUsers = [];
